...
const SHIPPED = [
  {
    ...
    body: 'Saldos calculados a partir dos lançamentos e uso do ciclo atual do cartão, respeitando o dia de fechamento.',
  },
  {
    title: 'Transações',
    body: 'Receitas e despesas com categorias, filtros por período, paginação e um fluxo dedicado para lançamentos sem categoria.',
  },
  {
    title: 'Painel',
    body: 'Saldo em caixa separado do que está em contas de investimento, comparação com o período anterior e os últimos 12 meses.',
  },
  {
    title: 'Recorrentes',
    body: 'Modelos mensais que geram ocorrências, com confirmação na data real, opção de pular e histórico preservado.',
  },
  {
    title: 'Investimentos',
    body: 'Carteira registrada manualmente com custo de aquisição por ativo. Sem cotação: os valores são os que você informou.',
  },
  {
    title: 'Metas',
    body: 'Objetivos com progresso atualizado por você — o app não deduz o valor a partir das transações.',
  },
  {
    title: 'Importação',
    body: 'CSV, OFX e XLSX do Banco Inter e no formato genérico, com pré-visualização, erros por linha e sem duplicar reimportações.',
  },
  {
    title: 'Backup local',
    body: 'Exportação completa em JSON e restauração no modo substituir ou mesclar, tudo dentro de uma transação.',
  },
];
    ...
    <main id="conteudo" className="mx-auto flex min-h-dvh max-w-5xl flex-col gap-12 px-5 py-12 sm:px-8">
      <section className="flex flex-col items-start gap-5">
        ...
        <p className="max-w-2xl text-md text-muted-foreground">
          Um gerenciador financeiro pessoal para acompanhar contas, cartões, lançamentos, despesas recorrentes,
          investimentos e metas — com os seus dados guardados no seu próprio banco.
        </p>
      ...
      <section aria-labelledby="radar">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Estes itens estão fora da V1. Estão listados aqui para deixar claro o que o app <strong>não</strong> faz hoje.
        </p>
        <ul className="mt-4 flex flex-col gap-2">
          {PLANNED.map((item) => (
            <li
              key={item}
              className="flex items-start gap-2 rounded-xl border border-dashed border-border-strong p-3 text-sm text-muted-foreground"
            >
